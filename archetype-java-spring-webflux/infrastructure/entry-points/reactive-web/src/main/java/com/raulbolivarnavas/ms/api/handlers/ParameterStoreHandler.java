...
public class ParameterStoreHandler {
    ...
    private final IParameterStoreGateway parameterStoreGateway;
    private final ObjectMapper objectMapper;

    public Mono<ServerResponse> getParameter(ServerRequest request) {
        String name = request.queryParam("name").orElseThrow(() ->
                new IllegalArgumentException("Missing 'name' query parameter"));

        return parameterStoreGateway.getParameter(name)
                .flatMap(value -> {
                    try {
